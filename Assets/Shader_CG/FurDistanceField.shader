Shader "Custom/FurDistanceField"{
	Properties{
	...
	CGINCLUDE
		...
		#define rotation 60. * (iGlobalTime);
		#define elevation 0.25 * cos(iGlobalTime * 2.5);
		...
		struct v2f {
			...
			vec3 ray_marching( vec3 origin, vec3 dir, float start, float end ) {
				...
				float depth = start;
				vec3 salida = vec3(end,end,end);
				vec3 dist = vec3(0.1,0.1,0.1);
			...
			vec3 shading( vec3 v, vec3 n, vec3 eye, vec3 lightMix) {
				...
				final += vec3( 0.9,0.9,0.9 ) * ( diffuse * lightMix.y + specular * lightMix.z);	
		...
		vec4 mainImage(vec2 fragCoord) {
			...
			if ( data.x < clip_far ) {
				...
				color = vec3(1.,1.,1.) * lightColor;
					
				vec4 c = vec4(0,0,0,0);
				
				for(int i=0; i<furLayers%1023; i++) {
					vec4 sampleCol;
