Shader "Unlit/Creepy Mountain"
{
	...
	CGINCLUDE
		...
		float bird(vec2 p) {
			p.x += iGlobalTime*.05;
			float v = floor(p.x / .4 - .2)*.7213548;
			float t = iGlobalTime*.05 + noise(vec2(v,v))*.7;
			p.x = mod(p.x, .4) - .2;
			p *= 2. - fmod(t, 1.)*2.;
			p.y += .6 - fmod(t, 1.);
		...
		float tree(vec2 p, float tx) {
			...
			p.x = mod(p.x, .2) - .1;
			float v = tx*1.72561;
			p *= 15. + noise(vec2(v,v))*10.;
			float ot = 1000.;
			float a = radians(-60. + noise(vec2(tx, tx))*30.);
			for (int i = 0; i<7; i++) {
				ot = min(ot, length(max(vec2(0.,0.), abs(p) - vec2(-a*.15, .9))));
		...
		float scene(vec2 p) {
			...
			if (noise(vec2(tx*3.75489, tx*3.75489))>.55) s = min(s, tree(p + vec2(0., terrain(tx)), .42 + tx*4.5798523));
			s = min(s, bird(p));
			return s;
		}
